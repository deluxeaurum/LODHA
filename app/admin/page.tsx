...
export default function AdminPage() {
  ...
  const fetchResidences = async () => {
    try {
      ...
      const res = await databases.listDocuments(
        ...
        [Query.limit(100), Query.orderAsc("order")]  // ✅ sorted by order
      );
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      setUploading(true);
      const uploaded = await storage.createFile(config.residenceBucketId, ID.unique(), file);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        order:    form.order,   // ✅ order included in payload
      };
      if (editingId) {
      ...
      notify(err instanceof Error ? err.message : "Save failed.", false);
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleEdit = (r: Residence) => {
    setForm({
      ...
      floors:   r.floors  ?? "",
      imageId:  r.imageId ?? "",
      order:    r.order   ?? 0,   // ✅ order loaded in edit form
    });
  ...
  const handleDelete = async (id: string, imageId?: string) => {
    try {
      setDeleting(id);
      if (imageId) {
        try { await storage.deleteFile(config.residenceBucketId, imageId); } catch {}
      }
  ...
  return (
    <div className="min-h-screen" style={{ background: "#FAF6EF", fontFamily: "'Montserrat', sans-serif" }}>
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed top-6 right-6 z-50 flex items-center gap-3 px-5 py-3.5 border shadow-lg
      ...
      {/* ══ NAV ══ */}
      <div className="sticky top-0 z-40 border-b border-[#B8952A]/15" style={{ background: "rgba(250,246,239,0.94)", backdropFilter: "blur(20px)" }}>
        ...
        <div className="max-w-[1320px] mx-auto px-6 lg:px-16 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3.5">
            ...
            <div className="leading-none">
              <p className="text-[#1C1610] tracking-[0.28em] font-light" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "20px" }}>LODHA</p>
          ...
          <div className="flex items-center gap-5">
            <a href="/admin/bulk" className="hidden sm:block text-[#1C1610]/30 hover:text-[#B8952A] text-[9px] tracking-[0.4em] uppercase font-light transition-colors duration-300">Bulk Upload</a>
            <a href="/residences" target="_blank" className="hidden sm:block text-[#1C1610]/35 hover:text-[#B8952A] text-[9px] tracking-[0.4em] uppercase font-light transition-colors duration-300">Live ↗</a>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-[1320px] mx-auto px-6 lg:px-16 py-14">
        ...
        {view === "form" && (
          <div className="max-w-3xl">
            <button onClick={goList} className="flex items-center gap-2 text-[#1C1610]/30 hover:text-[#B8952A] text-[9px] tracking-[0.4em] uppercase font-light transition-colors duration-300 mb-8">← Back</button>

            <div className="flex items-center gap-3 mb-2">
              <div className="w-5 h-px bg-[#B8952A]/60" />
              <span className="text-[#B8952A] text-[8px] tracking-[0.6em] uppercase font-light">{editingId ? "Edit Entry" : "New Entry"}</span>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-8">
              ...
              <div className="border border-[#B8952A]/15 p-8" style={{ background: "rgba(245,237,224,0.5)" }}>
                ...
                <div className="flex gap-6 items-start">
                  <button type="button" onClick={() => fileRef.current?.click()}
                    ...
                    {imagePreview ? (
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
                        {uploading ? (
                          <div className="flex gap-1">{[0,150,300].map((d) => <div key={d} className="w-1.5 h-1.5 rounded-full bg-[#B8952A] animate-bounce" style={{ animationDelay: `${d}ms` }} />)}</div>
                        ) : (
                          <>
                            <div className="w-8 h-8 border border-[#B8952A]/30 flex items-center justify-center"><span className="text-[#B8952A]/60 text-lg">+</span></div>
                  ...
                  <div className="flex-1 flex flex-col gap-4">
                    <div>
                      <p className="text-[#1C1610]/50 text-[11px] font-light leading-relaxed">Upload a property image. Stored in Appwrite Storage, file ID saved automatically.</p>
                      <p className="text-[#1C1610]/30 text-[10px] font-light mt-1.5">800×600px · JPG, PNG, WEBP · max 5MB</p>
                      {form.imageId && <p className="text-[#B8952A]/50 text-[9px] font-light mt-2 font-mono break-all">ID: {form.imageId}</p>}
                    </div>
                    <input ref={fileRef} type="file" accept="image/*" className="hidden"
                      onChange={(e) => { const file = e.target.files?.[0]; if (file) handleImageUpload(file); }} />
                    <div className="flex items-center gap-3 flex-wrap">
                      <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading}
                        className="px-5 py-2.5 text-[9px] tracking-[0.4em] uppercase font-light text-white disabled:opacity-50"
                        style={{ background: "linear-gradient(90deg,#B8952A,#D4B96A)" }}>
                        {uploading ? "Uploading…" : imagePreview ? "Replace Image" : "Choose File"}
                      </button>
                      {imagePreview && (
                        <button type="button" onClick={() => { setImagePreview(""); setForm((p) => ({ ...p, imageId: "" })); }}
              ...
              <div className="border border-[#B8952A]/12 p-8" style={{ background: "rgba(245,237,224,0.5)" }}>
                <div className="flex items-center gap-3 mb-6"><div className="w-4 h-px bg-[#B8952A]/50" /><p className="text-[#B8952A] text-[8px] tracking-[0.55em] uppercase font-light">Identity</p></div>
              ...
              {/* ── PRICING ── */}
              <div className="border border-[#B8952A]/12 p-8" style={{ background: "rgba(245,237,224,0.5)" }}>
                <div className="flex items-center gap-3 mb-6"><div className="w-4 h-px bg-[#B8952A]/50" /><p className="text-[#B8952A] text-[8px] tracking-[0.55em] uppercase font-light">Pricing</p></div>
                <Field label="Starting Price" value={form.price} onChange={f("price")} placeholder="₹ 12 Cr+" />
              </div>

              {/* ── CLASSIFICATION + SORT ORDER ── ✅ order field is back */}
              <div className="border border-[#B8952A]/12 p-8" style={{ background: "rgba(245,237,224,0.5)" }}>
                <div className="flex items-center gap-3 mb-6"><div className="w-4 h-px bg-[#B8952A]/50" /><p className="text-[#B8952A] text-[8px] tracking-[0.55em] uppercase font-light">Classification & Display</p></div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-x-8 gap-y-6">
                  ...
                  {/* ✅ Sort Order input — controls display order on /residences */}
                  <div className="flex flex-col gap-2">
                    <label className="text-[#1C1610]/40 text-[8px] tracking-[0.45em] uppercase font-light">Sort Order</label>
                    <input
                      type="number"
                      min={0}
                      value={form.order}
                      onChange={(e) => setForm((p) => ({ ...p, order: Number(e.target.value) }))}
                      className="w-full border border-[#B8952A]/15 bg-white/70 px-4 py-3 text-[#1C1610]/75 font-light outline-none focus:border-[#B8952A]/50 focus:bg-white transition-all duration-300"
                      style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "15px" }}
                    />
                    <p className="text-[#1C1610]/25 text-[9px] font-light">Controls order on /residences</p>
                  </div>
                </div>
              </div>
              ...
              <div className="border border-[#B8952A]/20 overflow-hidden" style={{ background: "#FBF7F0" }}>
                ...
                <div className="flex gap-0">
                  <div className="w-28 flex-shrink-0 relative" style={{ background: "linear-gradient(160deg,#F0E8D5,#EDE3CC)" }}>
                    {imagePreview ? <img src={imagePreview} alt="" className="absolute inset-0 w-full h-full object-cover" />
                      : <div className="absolute inset-0 flex items-center justify-center"><p className="text-[#B8952A]/20 text-[8px] tracking-[0.3em] uppercase font-light">No image</p></div>}
                  </div>
                  <div className="flex-1 p-5 flex flex-col gap-3">
                    <div>
                      <p className="text-[#B8952A] text-[7px] tracking-[0.5em] uppercase font-light">{form.location || "Location"}</p>
                      <p className="text-[#1C1610]/80 font-light mt-1" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "20px" }}>{form.name || "Project Name"}</p>
                    </div>
                    <div className="flex items-center gap-4">
                      <div>
                        <p className="text-[#1C1610]/30 text-[7px] tracking-[0.4em] uppercase font-light">Starting</p>
                        <p className="text-[#1C1610]/70 font-light" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "16px" }}>{form.price || "—"}</p>
                      </div>
                      ...
                      {form.status && (
                        <><div className="w-px h-8 bg-[#B8952A]/15" />
                        <span className={`text-[7px] tracking-[0.4em] uppercase font-light px-2.5 py-1 border ${statusStyle(form.status)}`}>{form.status}</span></>
                      )}
                    </div>
                  </div>
                </div>
        ...
        {view === "list" && (
          <>
            <div className="flex items-end justify-between mb-10">
              <div>
                <div className="flex items-center gap-3 mb-3"><div className="w-5 h-px bg-[#B8952A]/60" /><span className="text-[#B8952A] text-[8px] tracking-[0.6em] uppercase font-light">Database</span></div>
              ...
              <div className="flex items-center gap-4">
                <button onClick={fetchResidences} className="text-[#1C1610]/30 hover:text-[#B8952A] text-[9px] tracking-[0.4em] uppercase font-light transition-colors duration-300">↻ Refresh</button>
            ...
            {loading && (
              <div className="py-20 flex flex-col items-center gap-4">
                <div className="flex gap-1.5">{[0,150,300].map((d) => <div key={d} className="w-1.5 h-1.5 rounded-full bg-[#B8952A] animate-bounce" style={{ animationDelay: `${d}ms` }} />)}</div>
            ...
            {!loading && filtered.length === 0 && (
              <div className="py-20 text-center border border-[#B8952A]/10">
                <p className="text-[#1C1610]/20 text-[9px] tracking-[0.5em] uppercase font-light">{search ? "No matching properties" : "No residences added yet"}</p>
                {!search && <button onClick={() => setView("form")} className="mt-5 text-[#B8952A]/50 hover:text-[#B8952A] text-[9px] tracking-[0.4em] uppercase font-light transition-colors duration-300">+ Add first residence</button>}
              </div>
            )}
            ...
            {!loading && filtered.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                {filtered.map((r) => (
                  <div key={r.$id} className="group border border-[#B8952A]/12 overflow-hidden hover:border-[#B8952A]/35 hover:shadow-[0_4px_24px_rgba(184,149,42,0.10)] transition-all duration-500" style={{ background: "#FBF7F0" }}>
                    <div className="relative h-48 overflow-hidden" style={{ background: "linear-gradient(160deg,#F0E8D5,#EDE3CC)" }}>
                      {r.imageId ? (
                        <img src={resolveImage(r.imageId)} alt={r.name} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                      ) : (
                        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
                          <svg viewBox="0 0 120 160" className="w-16 h-20 opacity-20" fill="none">
                            <rect x="40" y="30" width="40" height="130" fill="#B8952A" />
                            {Array.from({ length: 10 }).map((_, i) => Array.from({ length: 4 }).map((_, j) => (
                              <rect key={`${i}-${j}`} x={44 + j * 9} y={36 + i * 13} width="6" height="9" fill={(i + j) % 3 === 0 ? "#B8952A" : "transparent"} opacity="0.6" />
                            )))}
                          </svg>
                      ...
                      <div className="absolute top-3 left-3">
                        <span className={`text-[7px] tracking-[0.4em] uppercase font-light px-2.5 py-1 border ${statusStyle(r.status)}`}>{r.status}</span>
                      </div>
                      <div className="absolute top-3 right-3">
                        <span className="text-[#1C1610]/30 text-[7px] tracking-[0.3em] uppercase font-light bg-white/60 px-2 py-1">{r.floors}</span>
                      </div>
                      {/* ✅ Sort order badge */}
                      <div className="absolute bottom-3 right-3">
                        <span className="text-[#1C1610]/40 text-[7px] tracking-[0.3em] font-light bg-white/70 px-2 py-1">#{r.order}</span>
                      </div>
                    </div>

                    <div className="p-5">
                      <p className="text-[#B8952A] text-[7px] tracking-[0.5em] uppercase font-light">{r.location}</p>
                      <p className="text-[#1C1610]/80 font-light mt-1.5 mb-3" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "20px" }}>{r.name}</p>
                      <div className="h-px bg-gradient-to-r from-[#B8952A]/20 to-transparent mb-3" />
                      <div className="flex justify-between items-end mb-4">
                        <div>
                          <p className="text-[#1C1610]/30 text-[7px] tracking-[0.4em] uppercase font-light">Starting</p>
                          <p className="text-[#1C1610]/70 font-light mt-0.5" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "17px" }}>{r.price}</p>
                        </div>
                      ...
                      <div className="flex items-center gap-3 pt-3 border-t border-[#B8952A]/[0.08]">
                        <button onClick={() => handleEdit(r)} className="flex-1 py-2 text-[9px] tracking-[0.4em] uppercase font-light text-[#B8952A] border border-[#B8952A]/25 hover:bg-[#B8952A] hover:text-white transition-all duration-300">Edit</button>
                        {deleteConfirm === r.$id ? (
                          <div className="flex-1 flex items-center justify-center gap-2 py-2 border border-red-300/40 bg-red-50">
                            <button onClick={() => handleDelete(r.$id, r.imageId)} disabled={!!deleting}
                            ...
                            <button onClick={() => setDeleteConfirm(null)} className="text-[#1C1610]/40 text-[9px] tracking-[0.35em] uppercase font-light hover:text-[#1C1610]/70">No</button>
                          </div>
                        ) : (
                          <button onClick={() => setDeleteConfirm(r.$id)} className="flex-1 py-2 text-[9px] tracking-[0.4em] uppercase font-light text-[#1C1610]/30 border border-[#1C1610]/10 hover:text-red-500 hover:border-red-300/40 transition-all duration-300">Delete</button>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            <div className="mt-6 flex items-center gap-3">
              ...
              <p className="text-[#1C1610]/25 text-[8px] tracking-[0.4em] uppercase font-light">
                {residences.length} {residences.length === 1 ? "property" : "properties"} · sorted by order · changes reflect live on /residences
              </p>
            </div>
          </>
        )}
